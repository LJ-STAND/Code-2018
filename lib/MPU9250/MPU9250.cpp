...
unsigned int MPU9250::WriteReg( uint8_t WriteAddr, uint8_t WriteData )
{
    ...
    select();
    SPI2.transfer(WriteAddr);
    temp_val=SPI2.transfer(WriteData);
    deselect();
...
void MPU9250::ReadRegs( uint8_t ReadAddr, uint8_t *ReadBuf, unsigned int Bytes )
{
    ...
    select();
    SPI2.transfer(ReadAddr | READ_FLAG);
    for(i = 0; i < Bytes; i++)
        ReadBuf[i] = SPI2.transfer(0x00);
    deselect();
...
void MPU9250::select() {
    ...
    SPI2.beginTransaction(SPISettings(my_clock, MSBFIRST, SPI_MODE3));
#ifdef CORE_TEENSY
