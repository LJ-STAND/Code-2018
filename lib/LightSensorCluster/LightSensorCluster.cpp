...
void LightSensorCluster::updateLengthCentre() {
    if (leftSensor > rightSensor) {
        centre = doubleMod((rightSensor + leftSensor - LS_NUM) / 2.0, LS_NUM);
        length = rightSensor - leftSensor + 1 + LS_NUM;
    } else {
...
double LightSensorCluster::getAngle() {
    return centre / LS_NUM_MULTIPLIER;
}

double LightSensorCluster::getLeftAngle() {
    return leftSensor / LS_NUM_MULTIPLIER;
}

double LightSensorCluster::getRightAngle() {
    return rightSensor / LS_NUM_MULTIPLIER;
}
